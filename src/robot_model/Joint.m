classdef Joint < handle
    ...
    properties
        ...
        ParentBody
        ChildBody
        Axis % Joint axis of roation, in body frame. [(1, 3) vector]
    end
    ...
    properties (SetAccess = private)
        ...
        %
        %   Default: eye(4)
        JointToParentTransform % Transfrom from joints to parent body
    ...
    methods
        ...
        function T = transformBody2Parent(obj)
            ...
            TP = obj.ParentBody.Joint.ChildToJointTransform; % Transform from previous joint to current joint
        ...
        function T = transformBody2ParentSymb(obj)
            % Find current config to initial config transform
            switch (obj.Type)
                case 'fixed'
                    TJ = eye(4);
    ...
    methods
        ...
        function setFixedTransform(obj, input, notation)
            ...
            %         set when adding the joint to the SpaceRobot using `addBody`
        ...
        % function set.ParentBody(obj, parentBody)
        %     obj.ParentBody = parentBody;

        %     % If parent is base, set JointToParentTransform to ManipToBaseTransform
        %     if obj.ParentBody.Id == 0
        %         obj.JointToParentTransform = obj.ParentBody.ManipToBaseTransform;
        %     end
        % end
    end
