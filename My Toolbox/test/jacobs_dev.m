...
Ab(1:6, :) = A{2}(:, :, 1)*A{1} * [R_b_I.', zeros(3, 3); zeros(3, 3), eye(3)]; % A_1_b = A_1_0 * A_0_b
...
A_dot_1_b = A_dot{2}(:, :, 1) * A{1} + A{2}(:, :, 1) * A_dot{1};% A_dot_1_b = A_dot_1_0 * A_0_b + A_1_0 * A_dot_0_b
Ab_dot(1:6, :) = A_dot_1_b;
...
%     fprintf('[Spart     Kin     DeNOC] (in Inertial frame)\n')
%     disp([t_S, blkdiag(R, R)*ti, blkdiag(R, R)*ti_denoc_i, ti_denoc_I])
...
function compare_jacs(sr, J_1, J_2)
    ...
    if allOk
        fprintf("All jacobians matching\n")
    else
        fprintf("ERROR, Jacobians not matching for bodies:\n")
