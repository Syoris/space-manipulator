...
spart_res = spart_script(sr, 'SR3.urdf', q, q_dot, q_ddot);

%% Initialize vals
sr_info = SR3_info_struct();
% 1 - Kinematics
[Rb, Ra, Rm] = RFunc_SR3(q);
...
%     end
    ...
    [R_4, ~] = tr2rt(sr.getTransform(sr.BodyNames{4}, 'TargetFrame', 'inertial', 'Symbolic', false));
    ...
    J_I{3} = blkdiag(R_3, R_3)*Jacobian('Body3', sr_info, A, {Rb, Ra});
    J_I{4} = blkdiag(R_4, R_4)*Jacobian('endeffector', sr_info, A, {Rb, Ra});
    
    J_i{1} = Jacobian('Body1', sr_info, A, {Rb, Ra});
    J_i{2} = Jacobian('Body2', sr_info, A, {Rb, Ra});
    J_i{3} = Jacobian('Body3', sr_info, A, {Rb, Ra});
    J_i{4} = Jacobian('endeffector', sr_info, A, {Rb, Ra});
    ...
    J_dot_i{3} = Jacobian_dot('Body3', sr_info, A, A_dot, {Rb, Ra}, qb_dot(4:6), Omega);
    J_dot_i{4} = Jacobian_dot('endeffector', sr_info, A, A_dot, {Rb, Ra}, qb_dot(4:6), Omega);
